define(['Vendor',
    ...
    Model = Class.extend({
        ...
        promiseResponse: function (data) {
            ...
            container.dayduration = daily.data[0].sunsetTime - daily.data[0].sunriseTime;
            container.currentDayPhaze = function () {


                var phazeDuration=container.sunsetTime-cur.time;
                var result = (cur.time < daily.data[0].sunsetTime )?phazeDuration/1000 :container.dayduration ;
            ...
            for (var i = 0; i < daily.data.length; i++) {
                ...
                container.week[i].variation = parseInt(Math.abs((container.week[i].minTemp / (container.week[i].maxTemp)) * 100) / 2);
            }
